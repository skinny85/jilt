...
It leverages the
[Java annotation processing API](https://docs.oracle.com/javase/8/docs/api/javax/annotation/processing/Processor.html)
to perform the code generation at compile time,
instead of runtime, which means using it doesn't incur any performance penalty.

Jilt's "killer features" compared to other tools in this same space are:
* Support for the Staged (sometimes also called Type-Safe, or Step, or Telescopic)
...
* The capability to generate Builders for any class,
  and without requiring any modifications to the target classes' source code.
* Seamless interoperability with other annotation processors,
  most noticeably [Lombok](https://projectlombok.org/).
...
Jilt also works with [Java 14+ Records](https://docs.oracle.com/en/java/javase/14/language/records.html):
...
dependencies {
    // ...
    compileOnly("cc.jilt:jilt:1.8.4") // Jilt is not needed at runtime
    annotationProcessor("cc.jilt:jilt:1.8.4") // you might also need this dependency in newer Gradle versions
}
```

If you're not using dependency managers, you can
[download the JAR directly](https://repo1.maven.org/maven2/cc/jilt/jilt/1.8.4/jilt-1.8.4.jar)
...
For example, for the `User` class from above:
...
which might make it difficult to find if the class has a large number of properties without an obvious order to them.
...
In addition to`style`, the `@Builder` annotation has a bunch of optional attributes that allow you to control
practically all aspects of the generated Builder:

* `className` allows you to change the generated Builder's name.
  The default name is `<BuiltClass>Builder`
  (so, if we're building a `Person` class, the default name will be `PersonBuilder`).
...
User user = UserBuilder.user(
    ...
    UserBuilder.Optional.username("johnnyd") // this is optional
);
```
...
(so, when the `@Builder.style` attribute is set to anything other than
`BuilderStyle.CLASSIC`, which is the default),
...
It has the following attributes (all of them optional):

* `outerName` allows you to change the name of the outer interface that the per-property interfaces
  will be generated in (this is in order not to pollute the global namespace).
  ...
  (unless the Builder class's name was changed with the `@Builder.className` attribute).
...
The simplest way to achieve that goal would be to make the constructor of the class package-private,
and make sure Jilt generates the Builder in the same package the target class is in
(which is the default behavior, if the `packageName` attribute of `@Builder` was not set),
so that the Builder can still access the constructor.

However, that simple solution has a downside:
the code inside the package of the target class can still instantiate it through its constructor.
If that is unacceptable in your situation,
the only way to completely prevent any code outside the target class from accessing its constructor is to make that constructor private.
That, in turn, makes it possible for the Builder to access that constructor only if it's a nested class inside the target class.
Unfortunately, annotation processors cannot modify existing classes,
only generate new ones
(yes, Lombok manages to sidestep this limitation,
but it does so in a way that is
[considered a hack](https://notatube.blogspot.com/2010/11/project-lombok-trick-explained.html)),
so Jilt cannot generate the Builder inside the target class.
...
With this, you can extend the Builder class in a nested class inside the target class,
and override the `build()` method to call the private constructor,
using the fields of the parent Builder as values of the properties.
You can also provide a static factory method in the target class that returns the Builder instance,
conventionally called just `builder()`,
which allows you to make the nested Builder class private.
...
public final class User {
    ...
    private static final class InnerBuilder extends UserBuilder {
        @Override
        public User build() {
...
would be to invoke the `User.builder()` static method,
...
In this case, the abstract method `@Builder` is placed on must be an instance method,
